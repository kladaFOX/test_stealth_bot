...
class BandsController < BotController
  ...
  def get_action
    if current_message.message.downcase == 'a'
      ...
      $redis.set("#{current_session.id}_previous_message", current_message.message) if @band.present?
      step_to state: :show
    else
  ...
  def show
    @band = Band.find($redis.get("#{current_session.id}_previous_message").to_i)
    send_replies
